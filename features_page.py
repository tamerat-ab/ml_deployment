...
def features():
        ...
        if x['object']:
            for i in  x['object']:
                ...
                for j in s:
                    if s.count(j)>1:
                        # f'You Selected "{i}" Criterion {s.count(j)} times!!!, only one criterion is permitted'
                        # st.error (f'You Selected "{i}" Criterion {s.count(j)} times!!!, only one criterion is permitted')
                        raise Exception (st.error(f'You Selected "{i}" Criterion {s.count(j)} times!!!, only one criterion is permitted'))
